...
function get_loyalty_title($loyalty_points) {
    $titles = [
        ...
        12 => ['name' => 'The Most Honorable of Tibia', 'points' => 10000],
    ];

    $matchingTitle = null;

    foreach ($titles as $titleId => $title) {
        if ($loyalty_points >= $title['points']) {
            $matchingTitle = $title['name'];
        }
    }
...
function convertAscii($input) {
    ...
    $length = strlen($input);
    if ($length > 0) {
        $char = ord($input[$length - 1]);
        $output .= (string)$char;
    }
    return $output;
}

function leftShiftBytes($decimal) {
    if (!is_int($decimal)) {
        return null;
    }
...
function undoLeftShiftBytes($decimal) {
    if (!is_int($decimal)) {
        return null;
    }
